        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
            ...
            <div class="space-y-6">
                <div class="space-y-2 text-center sm:text-left">
                    <span
                        class="text-xs font-extrabold uppercase tracking-[0.3em] text-[#0A4F78] dark:text-[#2A8FC2]">FORMULATION
                        CATALOG</span>
                    <h1 class="text-3xl sm:text-5xl font-black text-[#031827] dark:text-[#F6F5EF]">EXPLORE ALL PRODUCTS
                    </h1>
                </div>

                <div
                    class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-[#062B49] p-4 rounded-2xl border border-[#0A4F78]/15 shadow-sm">

                    <!-- Category Tabs -->
                    <div class="flex flex-wrap gap-2" id="shop-category-tabs">
                        <button onclick="filterShopCategory('ALL')"
                            class="shop-cat-btn px-4 py-2 rounded-xl text-xs uppercase font-extrabold tracking-wider bg-[#0A4F78] text-white cursor-pointer transition-all">ALL</button>
                        <button onclick="filterShopCategory('COGNITIVE')"
                            class="shop-cat-btn px-4 py-2 rounded-xl text-xs uppercase font-extrabold tracking-wider bg-transparent text-[#031827] dark:text-white border border-[#0A4F78]/20 hover:border-[#0A4F78] cursor-pointer transition-all">COGNITIVE</button>
                        <button onclick="filterShopCategory('ENERGY')"
                            class="shop-cat-btn px-4 py-2 rounded-xl text-xs uppercase font-extrabold tracking-wider bg-transparent text-[#031827] dark:text-white border border-[#0A4F78]/20 hover:border-[#0A4F78] cursor-pointer transition-all">ENERGY</button>
                        <button onclick="filterShopCategory('IMMUNITY')"
                            class="shop-cat-btn px-4 py-2 rounded-xl text-xs uppercase font-extrabold tracking-wider bg-transparent text-[#031827] dark:text-white border border-[#0A4F78]/20 hover:border-[#0A4F78] cursor-pointer transition-all">IMMUNITY</button>
                        <button onclick="filterShopCategory('WELLNESS')"
                            class="shop-cat-btn px-4 py-2 rounded-xl text-xs uppercase font-extrabold tracking-wider bg-transparent text-[#031827] dark:text-white border border-[#0A4F78]/20 hover:border-[#0A4F78] cursor-pointer transition-all">WELLNESS</button>
                    </div>

                    <!-- Sort Selector -->
                    <div class="flex items-center gap-2 w-full sm:w-auto justify-between sm:justify-end">
                        <span class="text-xs font-bold uppercase text-[#031827]/60 dark:text-[#F6F5EF]/60">SORT
                            BY:</span>
                        <select id="shop-sort-select" onchange="sortShopProducts(this.value)"
                            class="bg-[#F6F5EF] dark:bg-[#031827] border border-[#0A4F78]/30 rounded-xl p-2.5 text-xs font-extrabold text-[#031827] dark:text-white focus:outline-none focus:border-[#2A8FC2] cursor-pointer">
            ...
            <div id="shop-products-grid"
                class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 transition-opacity duration-300">
                ...
                @foreach ($products ?? [] as $product)
                    @php
                        ...
                        $pName = is_array($product)
                            ? (app()->getLocale() === 'ar' && !empty($product['name_ar'])
                                ? $product['name_ar']
                                : $product['name_en'] ?? ($product['name'] ?? ''))
                            : $product->name ?? ($product->name_en ?? '');
                        $pCategory = is_array($product)
                            ? (app()->getLocale() === 'ar' && !empty($product['category_ar'])
                                ? $product['category_ar']
                                : $product['category_en'] ?? ($product['category']['name'] ?? 'SUPPLEMENT'))
                        ...
                        $pShortDesc = is_array($product)
                            ? (app()->getLocale() === 'ar' && !empty($product['short_description_ar'])
                    ...
                    <div
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <h3 class="text-xl font-black text-[#031827] dark:text-[#F6F5EF]">
                                    <a href="{{ route('customer.product.show', $pSlug) }}"
                                        class="hover:text-[#67B34A] transition-colors">
                                        {{ $pName }}
                                    </a>
                                </h3>
                                <p
                                    class="text-xs text-[#031827]/70 dark:text-[#F6F5EF]/70 line-clamp-2 mt-1 font-medium">
                                    {{ $pShortDesc }}
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="pt-4 border-t border-[#0A4F78]/10 mt-5 space-y-3">
                            <a href="{{ route('customer.science.product', $pSlug) }}"
                                ...
                                <i
                                    class="fa-solid {{ app()->getLocale() === 'ar' ? 'fa-arrow-left' : 'fa-arrow-right' }} text-[10px] opacity-70 group-hover/btn:translate-x-1 rtl:group-hover/btn:-translate-x-1 transition-transform"></i>
                            </a>
                            ...
                            <div class="flex items-center justify-between gap-4 pt-1">
                                ...
                                <div class="flex gap-2">
                                    <button onclick="BLUEZONE_WISHLIST.toggle('{{ $pSlug }}')"
                                        class="p-3 rounded-xl border border-[#0A4F78]/30 hover:border-[#67B34A] text-[#0A4F78] dark:text-[#2A8FC2] hover:text-[#67B34A] transition-colors cursor-pointer"
                                        title="{{ app()->getLocale() === 'ar' ? 'المفضلة' : 'Wishlist' }}">
                                        <i class="fa-solid fa-heart"></i>
                                    </button>
                                    <button onclick="BLUEZONE_CART.add('{{ $pSlug }}', 1)"
                                        class="px-5 py-3 rounded-xl bg-[#0A4F78] hover:bg-[#062B49] text-white text-xs font-black uppercase tracking-wider transition-all btn-sheen cursor-pointer">
        ...
        function filterShopCategory(cat) {
            ...
            btns.forEach(b => {
                if (b.textContent.trim().toUpperCase() === cat.toUpperCase()) {
                    b.className =
                        'shop-cat-btn px-4 py-2 rounded-xl text-xs uppercase font-extrabold tracking-wider bg-[#0A4F78] text-white cursor-pointer transition-all';
                } else {
                    b.className =
                        'shop-cat-btn px-4 py-2 rounded-xl text-xs uppercase font-extrabold tracking-wider bg-transparent text-[#031827] dark:text-white border border-[#0A4F78]/20 hover:border-[#0A4F78] cursor-pointer transition-all';
                }
            });
            renderShopGrid();
        }
