<x-layouts.admin 
    ...
    :pageSubtitle="__('admin.customers.create_subtitle')"
    :breadcrumbs="[__('admin.menu.customers') => route('admin.customers.index'), __('app.actions.create') => route('admin.customers.create')]"
>
    <form method="POST" action="{{ route('admin.customers.store') }}">
        ...
        <div class="card" style="padding: 2rem; max-width: 800px;">
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                <x-forms.input name="name" :label="__('admin.customers.name')" placeholder="Dr. Rayan Al-Amri" :value="old('name')" required />
                <x-forms.input name="phone" type="tel" :label="__('admin.customers.phone')" placeholder="+966 54 111 5566" :value="old('phone')" />
            </div>

            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                <x-forms.input name="email" type="email" :label="__('admin.customers.email')" placeholder="client@bluezone.com" :value="old('email')" required />
